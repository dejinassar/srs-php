    ...
    if (empty($_REQUEST['stu_pass'])) {
        ...
        if (strlen($stu_pass) < 8) {
            ...
            if ($conn->query($sql) === TRUE) {
                ...
                //A flag in the session to indicate the user should be logged out
...
if (isset($_SESSION['logout']) && $_SESSION['logout'] === true) {
    // Clear the session and log out the user with JavaScript
    echo '<script type="text/javascript">logout();</script>';
}
?>

<script type="text/javascript">
    // JavaScript function to log out the user
    function logout() {
        // Clear the session (destroy the user's session)
        <?php session_destroy(); ?>
